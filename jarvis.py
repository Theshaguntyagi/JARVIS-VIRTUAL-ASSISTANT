...
class Jarvis:
    ...
    def wishMe(self) -> None:
        ...
        weather()
        speak('I am JARVIS 2 POINT O. Please tell me how can I help you SIR?')

    def sendEmail(self, to, content) -> None:
        server = smtplib.SMTP('smtp.gmail.com', 587)
        server.ehlo()
        server.starttls()
        server.login('theshaguntyagi@gmail.com', 'shaguntyagi@2003')
        server.sendmail('theshaguntyagi@gmail.com', to, content)
        server.close()
    ...
    def execute_query(self, query):
        ...
        if 'jarvis who made you' in query:
            speak("Yes Sir, my master BUILT ME AS MINOR PROJECT")
            ...
            if platform == "win32" or "darwin":
                speak('Shagun is my master. He created me couple of days ago')
            elif platform == "linux" or platform == "linux2":
                name = getpass.getuser()
                speak(name, 'is my master. He is running me right now')

        elif 'your name' in query:
            speak('My name is JARVIS 2 POINT O')
        
        elif 'who made you' in query:
            speak('I was created by my MASTER AS MINI PROJECT')

        elif 'birthday' in query:
            speak('I was BORN ON 26 NOVEMBER 2023 AT 11 O CLOCK')

        elif 'sibling' in query:
            speak('yes i have an elder sibling born 2 year before me.')
            
        elif 'stands for' in query:
            speak('J.A.R.V.I.S stands for JUST A RATHER VERY INTELLIGENT SYSTEM AND 2 POINT O FOR MY UPDATED VERSION')
            ...
            speak("you are not the good looking one please go and firstly see your face in mirror")
        ...
        elif "who is shagun tyagi" in query:
            speak("shagun tyagi is one of the  student of MIET college in the cse department in 3rss year.he is my owner,creator,master")

        elif "shagun friend zone" in query:
            speak("shagun friend zone is consist of few persons .all of them are important for the master")
        
        elif "what is" in query or "who is" in query or "calculate" in query:

            # Use the same API key
            # that we have generated earlier
            client = wolframalpha.Client("G77P9X-AAR2K5VQ75")
        ...
        elif 'personal website' in query:
            webbrowser.get('chrome').open_new_tab(
                'http://theshaguntyagi.me/PORTFOLIO/')
            
        elif 'favourite song' in query:
            webbrowser.get('chrome').open_new_tab(
                'https://youtu.be/zqGW6x_5N0k?si=Qpw0AqmB3DT60eFc')
...
if __name__ == '__main__':
    ...
    recognizer.read('Face-Recognition\\trainer\\trainer.yml')   #load trained model
    ...
    names = ['','Shagun']  #names, leave first empty bcz counter starts from 0
